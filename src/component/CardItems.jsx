...
export default function CardItems({item}) {
  return (<div  className='flex flex-col shadow-2xl'>
                <div className=' flex flex-col items-center justify-center text-center'>
                    ...
                    <img className='object-contain h-52 px-5 transition duration-300 ease-in-out hover:scale-110' src={item.image} />
                </div>
                <div className='px-6 py-3 flex flex-col text-start gap-3'>
                <h1 className='text-center'>{item.category} </h1>
                    <p className={`text-center text-white bg-blue-950`}>{item.rating.rate }</p>
